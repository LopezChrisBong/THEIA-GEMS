<template>
  <v-container fluid class="theia-view">
    <div class="page-header">
      <div><div class="page-heading">Stone Types</div><div class="page-sub">Manage gemstone type classifications</div></div>
      <div class="header-actions">
        <div class="search-wrap"><v-icon size="14" color="#9A7858">mdi-magnify</v-icon><input v-model="search" type="text" placeholder="Search stone types..." class="search-input-proto" /></div>
        <button class="btn-add" @click="addNew()"><v-icon size="13" color="white">mdi-plus</v-icon> Add Stone Type</button>
      </div>
    </div>
    <div class="cust-table-card"><div class="tbl-wrap">
      <table class="cust-table" v-if="!loading"><thead><tr><th>ID</th><th>Name</th><th>Description</th><th>Actions</th></tr></thead>
        <tbody>
          ...
          <tr v-if="filteredData.length===0"><td colspan="4"><div class="empty-state"><div class="empty-icon"><v-icon size="20" color="#9B6B3A">mdi-diamond-stone</v-icon></div><div class="empty-title">No stone types found</div></div></td></tr>
        </tbody></table>
    ...
    <StoneTypesDialog :data="updateData" :action="action" />
...
<script>
import StoneTypesDialog from "../../components/Dialogs/Forms/StoneTypesDialog.vue";
import eventBus from "@/eventBus";
export default {
  components:{StoneTypesDialog},
  ...
  mounted(){this.initialize();eventBus.on("closeStoneTypesDialog",()=>this.initialize())},
  beforeUnmount(){eventBus.off("closeStoneTypesDialog")},
  methods:{
    initialize(){this.loading=true;this.axiosCall("/stone-types","GET").then(r=>{if(r&&r.data)this.data=r.data}).catch(()=>{this.fadeAwayMessage={show:true,type:"error",header:"Error",message:"Failed to load",top:10}}).finally(()=>{this.loading=false})},
