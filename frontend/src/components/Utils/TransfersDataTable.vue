            ...
            <tr v-for="item in filteredData" :key="item.id">
              ...
              <td>
                <div class="act-btns">
                  ...
                  <button class="act-btn del" title="Delete" @click="deleteItem(item)" :disabled="item.status === 'completed'"><v-icon size="14">mdi-delete-outline</v-icon></button>
                </div>
              </td>
            </tr>
...
export default {
  ...
  mounted() {
    this.initialize();
    eventBus.on("closeTransfersDialog", () => this.initialize());
  },
  beforeUnmount() { eventBus.off("closeTransfersDialog"); },
  methods: {
    ...
    confirmDelete() {
      ...
      this.axiosCall("/transfers/" + this.deleteData.id, "DELETE")
        .then((r) => { if (r && (r.status === 200 || r.status === 204)) { this.fadeAwayMessage = { show: true, type: "success", header: "Success", message: "Transfer deleted", top: 10 }; this.dialogConfirmDelete = false; this.deleteData = null; this.initialize(); } })
