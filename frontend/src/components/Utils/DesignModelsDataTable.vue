<template>
  <v-container fluid class="theia-view">
    <div class="page-header">
      <div><div class="page-heading">Design Models</div><div class="page-sub">Manage jewelry design model names</div></div>
      <div class="header-actions">
        <div class="search-wrap"><v-icon size="14" color="#9A7858">mdi-magnify</v-icon><input v-model="search" type="text" placeholder="Search design models..." class="search-input-proto" /></div>
        <button class="btn-add" @click="addNew()"><v-icon size="13" color="white">mdi-plus</v-icon> Add Design Model</button>
      </div>
    </div>
    <div class="cust-table-card"><div class="tbl-wrap">
      <table class="cust-table" v-if="!loading"><thead><tr><th>ID</th><th>Model Name</th><th>Description</th><th>Actions</th></tr></thead>
        <tbody>
          <tr v-for="item in filteredData" :key="item.id">
            <td class="mono">{{ item.id }}</td><td><span class="cust-name">{{ item.modelName }}</span></td>
          ...
          <tr v-if="filteredData.length===0"><td colspan="4"><div class="empty-state"><div class="empty-icon"><v-icon size="20" color="#9B6B3A">mdi-palette-outline</v-icon></div><div class="empty-title">No design models found</div></div></td></tr>
        </tbody></table>
...
<script>
import DesignModelsDialog from "../../components/Dialogs/Forms/DesignModelsDialog.vue";
import eventBus from "@/eventBus";
export default {
  components:{DesignModelsDialog},
  ...
  computed:{filteredData(){if(!this.search)return this.data;const q=this.search.toLowerCase();return this.data.filter(i=>[i.modelName,i.description].filter(Boolean).some(f=>String(f).toLowerCase().includes(q)))}},
  watch:{options:{handler(){this.initialize()},deep:true}},
  mounted(){this.initialize();eventBus.on("closeDesignModelsDialog",()=>this.initialize())},
  beforeUnmount(){eventBus.off("closeDesignModelsDialog")},
  methods:{
    initialize(){this.loading=true;this.axiosCall("/design-models","GET").then(r=>{if(r&&r.data)this.data=r.data}).catch(()=>{this.fadeAwayMessage={show:true,type:"error",header:"Error",message:"Failed to load",top:10}}).finally(()=>{this.loading=false})},
