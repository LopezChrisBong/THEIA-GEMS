<template>
  <v-container fluid>
    ...
    <v-card flat class="mb-4 pa-4 rounded-xl header-card">
      <v-row align="center">
        <v-col cols="12" md="6">
          <h2 class="mb-1 font-weight-medium">Branches List</h2>
          <small class="text-medium-emphasis">
            Manage and view branches list
          </small>
        </v-col>
    ...
    <v-dialog v-model="dialogConfirmDelete" max-width="500">
      <v-card>
        ...
        <v-card-text class="text-body-1">
          Are you sure you want to delete the branch "{{ deleteData?.branchName }}"?
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn
...
export default {
  ...
  data: () => ({
    search: "",
    headers: [
      { title: "ID", value: "branchId", align: "start", width: 80 },
      { title: "Branch Name", value: "branchName", align: "start" },
      { title: "Code", value: "branchCode", align: "center", width: 120 },
      { title: "Address", value: "address", align: "start" },
      { title: "Phone", value: "phone", align: "center", width: 150 },
  ...
  mounted() {
    this.initialize();
    eventBus.on("closeBranchesDialog", () => {
      this.initialize();
    });
  },

  beforeUnmount() {
    eventBus.off("closeBranchesDialog");
  },
    ...
    initialize() {
      ...
      this.axiosCall("/branches", "GET")
        ...
        .catch((error) => {
          ...
          this.fadeAwayMessage.message = "Failed to load branches";
        })
        .finally(() => {
          this.loading = false;
        });
      ...
      this.axiosCall("/branches/" + this.deleteData.branchId, "DELETE")
        .then((res) => {
          if (res && (res.status === 200 || res.status === 204)) {
            ...
            this.fadeAwayMessage.message = "Branch deleted successfully";
            ...
            this.fadeAwayMessage.type = "error";
            this.fadeAwayMessage.header = "Error";
            this.fadeAwayMessage.message = "Failed to delete branch";
          }
        })
        .catch((error) => {
